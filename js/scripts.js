...
app.initModal = function () {
  ...
  var funcCreate = function () {
    modal.each(function () {
      var itemModal = $(this);
      itemModal.off('click.btn').on('click.btn', function (event) {
        event.preventDefault();
  ...
  if (slickCounter.length) {
    slickCounter
      .on('init', function (event, slick) {
        $(this).append(
          '<div class="slick-counter"><span class="current current01"></span> / <span class="total total01"></span></div>'
        );
